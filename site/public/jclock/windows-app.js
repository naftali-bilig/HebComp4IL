(function () {
  ...
  const UMID_STORAGE_KEY = "jclock.umid";
  const UMID_PATTERN = /^[0-9a-f]{24}$/i;

  function normalizeUmid(value) {
    const normalized = String(value || "").trim().replace(/^#/, "").toUpperCase();
    return UMID_PATTERN.test(normalized) ? normalized : null;
  }

  function umidFromUrl() {
    const ownValue = new URLSearchParams(window.location.search).get("umid");
    if (ownValue) return ownValue;
    try {
      return new URLSearchParams(window.parent.location.search).get("umid");
    } catch (_) {
      return null;
    }
  }

  function applyUmid(value, persist = true) {
    const umid = normalizeUmid(value);
    if (!umid) return false;

    // UMID order: moon hour, moon day, sun hour, sun day.
    const colors = Array.from({ length: 4 }, (_, index) => `#${umid.slice(index * 6, index * 6 + 6)}`);
    const root = document.documentElement;
    root.style.setProperty("--umid-general-text", colors[0]);
    root.style.setProperty("--umid-general-background", colors[1]);
    root.style.setProperty("--umid-button-text", colors[2]);
    root.style.setProperty("--umid-button-background", colors[3]);
    root.dataset.umidTheme = "true";
    if (persist) {
      try { window.localStorage.setItem(UMID_STORAGE_KEY, umid); } catch (_) { /* Storage may be disabled. */ }
    }
    return true;
  }

  function initializeUmid() {
    const urlUmid = umidFromUrl();
    let storedUmid = null;
    try { storedUmid = window.localStorage.getItem(UMID_STORAGE_KEY); } catch (_) { /* Storage may be disabled. */ }
    applyUmid(urlUmid || storedUmid, Boolean(urlUmid));
  }

  window.setJClockUmid = applyUmid;
  window.addEventListener("message", function (event) {
    if (event.origin !== window.location.origin) return;
    if (event.data?.type === "jclock.umid") applyUmid(event.data.umid);
  });
