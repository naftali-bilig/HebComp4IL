...
const INTERACTIVE_SELECTOR = "button, input, select, textarea, a, [role='button'], [role='slider'], [contenteditable='true'], .hidden-starr-panel";
...
function startMusicFromGesture() {
  ...
  backgroundMusic.resumeAudioContext().catch(() => {});
  startMusic();
...
function startMusicFromBackground(event) {
  if (shouldIgnoreTap(event)) return;
  const now = performance.now();
  if (now - lastBackgroundActivationAt < 350) return;
...
window.addEventListener("pointerup", startMusicFromBackground, { capture: true, passive: true });
window.addEventListener("click", startMusicFromBackground, { capture: true });
